...
export default async function ParentHolidaysPage() {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 1000 }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>🏖️ Holiday Information</h1>
        ...
        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', gridColumn: '1 / -1' }}>
          ...
          <div style={{ padding: '1rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px,1fr))', gap: '0.75rem' }}>
            {holidays.length === 0 ? (
              ...
              return (
                <div key={h.id} style={{ padding: '12px', borderRadius: 10, background: t.bg, border: `1px solid ${t.color}22`, display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 24, flexShrink: 0 }}>{t.emoji}</span>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: 0, lineHeight: 1.2 }}>{h.title}</p>
          ...
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '0.75rem', maxHeight: 350, overflowY: 'auto' }}>
              {homework.length === 0 ? (
                ...
                return (
                  <div key={h.id} style={{ padding: '12px', borderRadius: 8, border: '1px solid #f1f5f9', marginBottom: 8, background: overdue ? '#fff8f8' : 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6, marginBottom: 4 }}>
                      <div>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 10, background: '#eef2ff', color: '#4f46e5', marginRight: 4 }}>{h.subject}</span>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>{h.title}</p>
                    {h.description && <p style={{ fontSize: 12, color: '#475569', margin: 0 }}>{h.description}</p>}
          ...
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '0.75rem', maxHeight: 350, overflowY: 'auto' }}>
              {materials.length === 0 ? (
                ...
                <div key={m.id} style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid #f1f5f9', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 24, flexShrink: 0 }}>{MAT_ICONS[m.type] ?? '📄'}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: 0 }}>{m.title}</p>
                    <p style={{ fontSize: 11, color: '#64748b', margin: '2px 0 0' }}>{m.subject} · {m.teacher?.full_name}</p>
                    {m.content && <p style={{ fontSize: 12, color: '#475569', margin: '4px 0 0' }}>{m.content}</p>}
                  </div>
                  {m.url && (
