...
export default function ParentAttendance({ children_, attendance }: { children_: Child[]; attendance: AttRecord[] }) {
  ...
  return (
    <div className="responsive-page-container">
      ...
      {children_.length === 0 ? (
        <div className="card" style={{ padding: '2.5rem 1rem', textAlign: 'center', color: '#94a3b8' }}>No children linked to your account.</div>
      ) : (
        <>
          {children_.length > 1 && (
            <div style={{ marginBottom: '1.25rem' }}>
              <select value={selectedChild} onChange={e => setSelected(e.target.value)}
                style={{ width: '100%', maxWidth: 360, padding: '9px 12px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 13, background: '#f8fafc', fontFamily: 'inherit', boxSizing: 'border-box' }}>
          ...
          {child && (
            <div style={{ background: '#fdf4ff', border: '1px solid #e9d5ff', borderRadius: 10, padding: '10px 14px', marginBottom: '1.25rem', fontSize: 12.5, color: '#6b21a8', lineHeight: 1.4 }}>
              <strong>{child.full_name}</strong> &nbsp;·&nbsp; {child.class_name} — Sec {child.section} &nbsp;·&nbsp; #{child.roll_number}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 100px), 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {[
              ...
              <div key={i} className="card" style={{ padding: '0.875rem 1rem', background: s.bg }}>
          ...
          <div className="card" style={{ overflow: 'hidden' }}>
            ...
            {myAtt.length === 0
              ? <div style={{ padding: '2.5rem 1rem', textAlign: 'center', color: '#94a3b8' }}>No attendance records found.</div>
