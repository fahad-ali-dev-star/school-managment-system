...
export default async function TeacherDashboard() {
  ...
  return (
    <div className="responsive-page-container">
      ...
      {classAssigned ? (
        <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '10px 14px', marginBottom: '1.25rem', fontSize: 12.5, color: '#15803d', lineHeight: 1.4 }}>
          Your assigned class: <strong>{classAssigned}</strong>
          {teacherRow?.subject && <> &nbsp;·&nbsp; Subject: <strong>{teacherRow.subject}</strong></>}
        </div>
      ) : (
        <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', marginBottom: '1.25rem', fontSize: 12.5, color: '#92400e', lineHeight: 1.4 }}>
          No class assigned yet. Please contact the admin.
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 120px), 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
        {[
          ...
          <div key={i} className="card" style={{ padding: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
              <p style={{ fontSize: 10, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>{s.label}</p>
              <span style={{ fontSize: 16 }}>{s.icon}</span>
            </div>
            <p style={{ fontSize: '1.45rem', fontWeight: 700, color: s.color, margin: 0 }}>{s.value}</p>
          </div>
        ))}
      </div>

      <div className="card" style={{ padding: '1.25rem' }}>
        <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: '0.875rem', color: '#0f172a' }}>Quick Actions</h2>
