...
export default function TeacherExamsManager({
      ...
      <div className="responsive-page-container">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '1.5rem' }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ fontSize: 12.5, color: '#64748b', margin: '2px 0 0', wordBreak: 'break-word' }}>{activeExam.title} · {activeExam.class_name} Section {activeExam.section}</p>
          </div>
        </div>
        ...
        {loadingStudents
          ...
          : subjects.length === 0
          ? <div className="card" style={{ padding: '2rem 1rem', textAlign: 'center', color: '#94a3b8' }}>No subjects added to this exam.</div>
          : subjects.map(subject => (
            <div key={subject.id} className="card" style={{ marginBottom: '1.25rem', overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
                ...
                <span style={{ fontSize: 11.5, color: '#94a3b8' }}>Max: {subject.total_marks} | Pass: {subject.passing_marks}</span>
              </div>
                    ...
                    {students.map((st: any, i: number) => {
                      ...
                      return (
                        <tr key={st.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 ? '#fafafa' : 'white' }}>
                          <td style={{ padding: '8px 12px', fontFamily: 'monospace', color: '#6366f1', fontSize: 12 }}>{st.roll_number}</td>
                          <td style={{ padding: '8px 12px', fontWeight: 500, color: '#0f172a' }}>{st.full_name}</td>
                          <td style={{ padding: '8px 12px' }}>
                            <input type="number" min="0" max={subject.total_marks} value={val} placeholder="0"
                              ...
                              style={{ ...inp, width: 85, padding: '6px 8px', fontSize: 13 }} />
                          </td>
                          <td style={{ padding: '8px 12px' }}>
        ...
        {!loadingStudents && subjects.length > 0 && students.length > 0 && (
          <button onClick={handleSaveMarks} disabled={savingMarks} style={{
            padding: '11px 24px', background: savedMarks ? '#16a34a' : savingMarks ? '#a5b4fc' : '#4f46e5',
  ...
  return (
    <div className="responsive-page-container">
      ...
      {exams.length === 0
        ? <div className="card" style={{ padding: '2.5rem 1rem', textAlign: 'center', color: '#94a3b8' }}>No exams found for your class yet.</div>
        : <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {exams.map(exam => {
            const ss = STATUS_STYLE[exam.status] ?? STATUS_STYLE.upcoming
            const subjects: Subject[] = exam.subjects ?? []
            return (
              <div key={exam.id} className="card" style={{ padding: '1.15rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                      <h2 style={{ fontSize: 14.5, fontWeight: 600, color: '#0f172a', margin: 0, wordBreak: 'break-word' }}>{exam.title}</h2>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: ss.bg, color: ss.color }}>{ss.label}</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap', width: '100%', maxWidth: 'max-content' }}>
                    {exam.status !== 'published' && (
                      <button onClick={() => openMarksEntry(exam)} style={{ padding: '6px 12px', border: '1px solid #bbf7d0', borderRadius: 6, background: '#f0fdf4', cursor: 'pointer', fontSize: 12, color: '#16a34a', fontFamily: 'inherit', fontWeight: 500 }}>
                        ✏ Enter / Edit Marks
                      </button>
                    )}
                    {(exam.status === 'ongoing' || exam.status === 'upcoming') && (
                      <button onClick={() => handleStatusChange(exam, 'completed')} style={{ padding: '6px 12px', border: '1px solid #ddd6fe', borderRadius: 6, background: '#f5f3ff', cursor: 'pointer', fontSize: 12, color: '#6d28d9', fontFamily: 'inherit', fontWeight: 500 }}>
                        ✓ Submit to Admin
                      </button>
                    )}
                    {exam.status === 'published' && (
                      <button onClick={() => openMarksEntry(exam)} style={{ padding: '6px 12px', border: '1px solid #e0e7ff', borderRadius: 6, background: '#eef2ff', cursor: 'pointer', fontSize: 12, color: '#4f46e5', fontFamily: 'inherit', fontWeight: 500 }}>
                        👁 View Marks
                      </button>
                    )}
                  </div>
